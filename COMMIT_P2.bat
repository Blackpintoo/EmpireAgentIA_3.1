@echo off
setlocal
title Commit P2 - risque uniforme
cd /d "%~dp0"

echo.
echo ==========================================================
echo   P2 : DIMENSIONNEMENT UNIFORME (250 USD / trade)
echo ==========================================================
echo.
...
echo.
echo       Attendu : environ 15 lignes sur 2 fichiers (config.yaml, overrides.yaml).
...
echo [3/4] Commit
git add -- config/config.yaml config/overrides.yaml
git commit -F "_transfert\msg_p2.txt"
...
:pas_bon_dossier
echo [ERREUR] Aucun depot git ici. Lance depuis C:\EmpireAgentIA_3
goto :fin
